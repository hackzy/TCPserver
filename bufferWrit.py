class 写封包:
    ...
    def 写字节型(self,buffer):
         self.data += bytes(buffer)
         return
    
    def 写短整数型(self,buffer,反转=False):
        if 反转:
            buffer = int.to_bytes(buffer,2,'big')
        self.data += int.to_bytes(buffer,2,'little')
        return
    
    def 写整数型(self,buffer,反转=False):
        if 反转:
            buffer = int.to_bytes(buffer,4,'big')
        self.data += int.to_bytes(buffer,4,'little')
        return
