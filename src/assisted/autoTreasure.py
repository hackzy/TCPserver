...
class AutoTreasure:
    ...
    def getTreasurePot(self,user,server):
        return server.基础功能.getItemPot(user,'藏宝图')
    def getHoe(self,user,server):
        return server.基础功能.getItemPot(user,'锄头')
    
    def buyHoe(self,user,server):
        xianlingkaPot = server.基础功能.getItemPot(user,'仙灵卡')
        if xianlingkaPot == 0:
            server.基础功能.商城购买道具(user,'仙灵卡')
            xianlingkaPot = server.基础功能.getItemPot(user,'仙灵卡')
        server.客户端发送(user.fuzhu.使用物品(xianlingkaPot),user)
        time.sleep(1)
        server.客户端发送(server.基础功能.对话点击(user.gamedata.角色id,'打开商店'),user)
        time.sleep(1)
    ...
    def startTreasure(self,user,server):
        ...
        if treasPot == 0:
            self.flag = False
            server.客户端发送(server.基础功能.中心提示('当前背包里没有藏宝图！'),user)
        ...
        for task in user.gamedata.任务:
            if task.find('宝藏') != -1:
                break
        if user.gamedata.任务[task] == '' or task.find('宝藏') == -1:
